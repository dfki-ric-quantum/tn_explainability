...
from tn.ttn.tree import random_tree
...
def get_config(hyper_params: dict) -> config_dict.FrozenConfigDict:
    """Turn hyper-parameters into TTN train/model config.
...
class Objective:
    """Hyper-parameter tuning objective for TTN training."""
    ...
    def __call__(self, trial: optuna.trial.Trial) -> float:
        ...
        for fold_idx, (train_idxs, _) in enumerate(spliter.split(x, self.y)):
            ...
            tree = random_tree(self.n_features, cfg.bond_dim, cfg.d, subkey)
            tree.left_canonicalize()

            train_tree(tree, x[train_idxs], cfg=cfg)

            loss_fn = NLLFunctor(tree.shapes, tree.leaf_mask, tree.d)
            scores = loss_fn(x[train_idxs], tree)
